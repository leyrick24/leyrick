<template>
	<div class="row align-items-center padcenter">
		...
		<div class='col-md-2'>
		    <input @change="toggleOnOffAll('all')" v-model="switchStatusAll" class="tgl tgl-ios" :id="'wall_switch_2_all' + device['key']" type="checkbox"/>
		    <label  class="tgl-btn" :for="'wall_switch_2_all' + device['key']"></label>
		</div>
	</div>
</template>
	...
	export default{
		...
		data(){
			return{
				...
				device_type:'wall_switch_2'				//Device Type
		...
		methods:{
			...
			//Function Description: Toggle switch all based on individual swtich status
			//Param: switchStatus
			checkAllButton(){
				for (var i in this.switchStatus)
