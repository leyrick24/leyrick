	...
	<div class="row align-items-center pad-50">
		...
		<div class='col-md-9'>
			<div class='row'>
				...
				<div class='col-md-8'>
					<!-- @change trigger toggleOnOff -->
				    <input @change='toggleOnOff(2)' v-model="switchStatus[1]" class="tgl tgl-ios" :id="'embedded_switch_2' + device['key']" type="checkbox"/>
				    <label  class="tgl-btn" :for="'embedded_switch_2' + device['key']"></label>
			    </div>
				<div class='col-md-4'><label>{{device['DATA'][2]['device_name']}}</label></div>
				<div class='col-md-8'>
					<!-- @change trigger toggleOnOff -->
				    <input @change='toggleOnOff(3)' v-model="switchStatus[2]" class="tgl tgl-ios" :id="'embedded_switch_3' + device['key']" type="checkbox"/>
				    <label  class="tgl-btn" :for="'embedded_switch_3' + device['key']"></label>
			    </div>
			</div>
		</div>
		<div class='col-md-2'>
		    <input @change="toggleOnOffAll('all')" v-model="switchStatusAll" class="tgl tgl-ios" :id="'embedded_switch_3_all' + device['key']" type="checkbox"/>
		    <label  class="tgl-btn" :for="'embedded_switch_3_all' + device['key']"></label>
		</div>
	</div>
</template>
	...
	export default{
		...
		data(){
			return{
				...
				device_type:'embedded_switch_3'	//Device Type
