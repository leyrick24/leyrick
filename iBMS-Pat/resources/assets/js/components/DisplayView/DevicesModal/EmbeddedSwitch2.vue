<template>
	<div class="row align-items-center padcenter">
		<div class='col-md-1'>
		</div>
		<div class='col-md-9'>
			<div class='row'>
		...
		<div class='col-md-2'>
		    <input @change="toggleOnOffAll('all')" v-model="switchStatusAll" class="tgl tgl-ios" :id="'embedded_switch_2_all' + device['key']" type="checkbox"/>
		    <label  class="tgl-btn" :for="'embedded_switch_2_all' + device['key']"></label>
		</div>
	</div>
</template>
	...
	export default{
		...
		data(){
			return{
				...
				device_type:'embedded_switch_2'				//Device Type
		...
		methods:{
			...
			commandDevice(command,value){
				...
				// this.$bus.$emit('returnDeviceControlData',{'KEY':this.device['key'],
